...
import ch.ichristen.avroUtil.serde.depricated.AvroSerializer;
...
public class OldSerdeTest {
    ...
    public void testSingleSpecificBinarySerde() {
        ...
        AvroSerializer<TestObjectParent> ser = new AvroSerializer<>(true);
        ...
        AvroDeserializer<TestObjectParent> deser = new AvroDeserializer<>(true);
        if (result != null) {
            try {
    ...
    public void testSingleSpecificJsonSerde() {
        ...
        AvroSerializer<TestObjectParent> ser = new AvroSerializer<>(false);
        final TestObjectParent testObject = buildTestObject(0);
        byte[] result = null;
        try {
            result = ser.serialize(testObject);
